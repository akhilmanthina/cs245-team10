...
# HyDE hypothetical context generation
def generate_hypothetical_context(query, llm, tokenizer):
    # Use the LLM to generate a hypothetical context
    system_prompt = "You are a helpful assistant. You are given the following question. Generate a plausible response such that someone who read the response could answer the question."
    ...
    responses = llm.generate(
        formatted_prompt,
        vllm.SamplingParams(
            n=1, top_p=0.9, temperature=0.7, max_tokens=100
        )
    )
...
# Combination of HyDE with original query embedding cosine similarity score comparison
class HybridHyDEModel:
    """
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        # Calculate embeddings for hypothetical contexts instead of queries
        hypothetical_contexts = [generate_hypothetical_context(query, self.llm, self.tokenizer) for query in queries]
        hyde_embeddings = self.calculate_embeddings(hypothetical_contexts)
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            hyde_embedding = hyde_embeddings[_idx]
            ...
            cosine_scores_hyde = (relevant_chunks_embeddings * hyde_embedding).sum(1)

            alpha = 0.3 # Weight that can be tuned
            combined_scores = alpha * cosine_scores + (1 - alpha) * cosine_scores_hyde

            # and retrieve top-N results.
            retrieval_results = relevant_chunks[
                (-combined_scores).argsort()[:NUM_CONTEXT_SENTENCES]
            ]
