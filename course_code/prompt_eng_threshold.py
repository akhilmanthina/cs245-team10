...
class ThresholdPromptModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            for chunk, score in zip(relevant_chunks, cosine_scores):
                if score >= threshold:  # Keep only non-negative scores
                    filtered_chunks.append(chunk)
                    filtered_scores.append(score)

            # Retrieve top-N results based on filtered scores
            sorted_indices = np.argsort(filtered_scores)[::-1]  # Sort in descending order
            top_k_indices = sorted_indices[:NUM_CONTEXT_SENTENCES]

            # and retrieve top-N results and add to batch
            batch_retrieval_results.append([filtered_chunks[i] for i in top_k_indices])
            batch_cos_scores.append([filtered_scores[i] for i in top_k_indices])
            
        # Prepare formatted prompts from the LLM        
        formatted_prompts = self.format_prompts(queries, query_times, batch_retrieval_results, batch_cos_scores)
    ...
    def format_prompts(self, queries, query_times, batch_retrieval_results=[], batch_cos_scores=[]):
        """
        ...
        """        
        system_prompt = """
        You are provided with a question and several references, each accompanied by a relevance score ranging from -1 to 1:
        - 1: Extremely relevant to the question.
        - 0: Neutral or moderately related.
        - -1: Not relevant to the question.

        Your task is to:
        1. Prioritize highly relevant references (those with scores closer to 1) when forming your response.
        2. Answer the question succinctly, using the fewest words necessary.
        3. If the references do not contain enough information to answer the question, respond with "I don't know".
        4. Avoid guessing or making assumptions. "I dont' know" is a good answer compared to giving unreliable information. 
        Do not provide reasoning or explanations for your answers.
        """
        formatted_prompts = []
        ...
        for _idx, query in enumerate(queries):
            ...
            cos_scores = batch_cos_scores[_idx]
            ...
            if len(retrieval_results) > 0:
                ...
                for score, snippet in zip(cos_scores, retrieval_results):
                    references += f"- Score: {score:.2f}, Reference: \"{snippet.strip()}\"\n"
            ...
            user_message 
            user_message += f"Using the references listed and their scores, answer the following question: \n"
