...
class RAGHyDEModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        query_embeddings = self.calculate_embeddings(hypothetical_contexts)

        # for testing 
        query_embeddings_og = self.calculate_embeddings(queries)
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            # for testing
            query_embedding_og = query_embeddings_og[_idx]
            ...
            # for testing
            cosine_scores_og = (relevant_chunks_embeddings * query_embedding_og).sum(1)
            self.avg_cos_sim_diff += (np.sum(cosine_scores) - np.sum(cosine_scores_og)) # we want this to be positive 
            self.total_pred += 1

            print(self.avg_cos_sim_diff / self.total_pred)
